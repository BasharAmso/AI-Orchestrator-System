...
This guide walks you through using Bashi from start to finish. No programming experience required — you'll describe what you want in plain language and the system handles the rest.
...
Before using Bashi, make sure you have:
...
- **A copy of Bashi** — Either cloned from the template repository or copied using the `/clone-framework` command from an existing project.
...
Open your project folder in VS Code. If you copied Bashi template into a folder called `my-recipe-app`, open that folder.
...
Bashi has two separate control systems. Framework Mode controls how much planning happens. Run Mode controls how fast work executes.
...
Bashi has three speed settings. You control which one is active.
...
| `/clone-framework <path>` | Copy Bashi to a new project directory. Add `--upgrade` to update an existing project's framework files. |
...
Bashi is built from ten building blocks:
...
Bashi is designed to be reused across projects. You can copy it to new projects or upgrade existing ones.

### Setting Up a New Project

From any project that already has Bashi:
